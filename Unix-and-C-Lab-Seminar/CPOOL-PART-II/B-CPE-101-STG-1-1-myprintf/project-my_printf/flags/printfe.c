...
int isneg_or_zeromin(double nb, char subflag)
{
...
int printexpomin(double temp, int countnb)
{
    int count = 0;
...
int printfe(va_list list, char subflag, int spaces_o_zero)
{
    ...
    if (nb == 0.0 || nb < 0.0 && specialcase(nb) != 1)
        return (isneg_or_zeromin(nb, subflag));
    ...
    count = count + printexpomin(temp, countnb);
    return (count);
}
